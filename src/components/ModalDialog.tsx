...
export function ModalDialog() {
  ...
  return (
    <Dialog
      open={useDialogStore((state) => state.isDialogVisible)}
      onClose={hideDialog}
    >
      ...
      <DialogContent>
        ...
        <DialogActions>
        <Button onClick={() => { resolveDialog?.(true); hideDialog(); }} variant="contained">OK</Button>
        {useDialogStore((state) => state.isDialogTwoButtons) &&
        <Button onClick={() => { resolveDialog?.(false);  hideDialog(); }} variant="outlined">Cencel</Button>}
      </DialogActions>
    </Dialog>
  );
}